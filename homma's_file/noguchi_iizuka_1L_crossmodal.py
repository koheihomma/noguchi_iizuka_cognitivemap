...
class Model(nn.Module):
	def __init__(self):
		...
		self.Low = nn.GRUCell(vision_feature_size+motion_feature_size, hidden_size)
		self.fc2_v = nn.Linear(vision_feature_size, vision_input_size)
		self.fc2_m = nn.Linear(motion_feature_size, motion_input_size)


	def forward(self, v_0, m_0, hidden=None):
		if hidden is None:
			hidden = torch.zeros(1, v_0.shape[1], self.hidden_size).to(device)
		#PVM
		...
		if v_0.shape==(1, 1, 768) and m_0.shape==(50, 1, 2):
			...
			for i in range(time_len):
				...
				L_input = torch.cat((f_v[0], f_m[i]), dim=1)
				#print(L_input.shape)
				hidden = self.Low(L_input, hidden[0])
				hidden = hidden.view(1, hidden.shape[0], hidden.shape[1])
				f_v_n = F.elu(hidden[:, :, :self.vision_feature_size])
				f_m_n = F.elu(hidden[:, :, self.vision_feature_size:])
				v_n = torch.sigmoid(self.fc2_v(f_v_n))
				m_n = torch.tanh(self.fc2_m(f_m_n))
				v_list.append(v_n)
				m_list.append(m_n)
		...
		if v_0.shape==(50, 1, 768) and m_0.shape==(1, 1, 2):
			...
			for i in range(time_len):
				...
				L_input = torch.cat((f_v[i], f_m[0]), dim=1)
				hidden = self.Low(L_input, hidden[0])
				hidden = hidden.view(1, hidden.shape[0], hidden.shape[1])
				f_v_n = F.elu(hidden[:, :, :self.vision_feature_size])
				f_m_n = F.elu(hidden[:, :, self.vision_feature_size:])
...
def train():
	...
	with h5py.File(hdfpath,'r') as f:
		...
		for n in range(100):
			...
			for motion_list, vision_list in zip(motion_lists, vision_lists):
				...
				vision_output, motion_output=model(v_0.view(50, 1, 768), m_0.view(50, 1, 2))

				v_50_list=[]
				m_50_list=[]
				v_50_list.append(vision_output[49])
				m_50_list.append(motion_output[49])
				...
				for i in range(18):
					...
					v_50_list.append(vision_output1[49])
					m_50_list.append(motion_output1[49])
					...
					#POM
					vision_output2, motion_output2=model(v_50_list[i].view(1, 1, 768), m_0.view(50, 1, 2))
					...
					#POV
					vision_output3, motion_output3=model(v_0.view(50, 1, 768), m_50_list[i].view(1, 1, 2))
	...
	torch.save(model.state_dict(), "gru_1L_crossmodal.pt")
	f = open('loss_1L_crossmodal.txt', 'wb')
...
def test():
	...
	model = Model().to(device)
	model.eval()
	model.load_state_dict(torch.load('gru_1L_crossmodal.pt'))
	...
	with h5py.File(hdfpath,'r') as f:
		...
		f_v = open('vision_output_1L_crossmodal.txt', 'wb')
		f_m = open('motion_output_1L_crossmodal.txt', 'wb')
