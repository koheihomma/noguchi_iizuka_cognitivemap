...
f_m = open("./motion_output_sample.txt","rb")
f_v = open("./vision_output_sample.txt","rb")
f_h5 = h5py.File('data_homma.h5', 'r')

motion = pickle.load(f_m)[idx].clone().detach()
position = f_h5['position'][mode][()][idx] * 5
vision = pickle.load(f_v)[idx].clone().detach()
vision = vision.reshape(-1,height, width * num_camera, channel)
#vision = vision.reshape(-1, channel, width * num_camera, height)
#vision = vision.permute(0, 2, 3, 1)
